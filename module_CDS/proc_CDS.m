...
[lfp_bwb, lfp_bwa]                  = butter(2, [0.1 200]/(recdev.sampling_rate/2), 'bandpass');
...
try
    ...
    test_fid        = fopen(recdev.in_file_path + "\amp-A.dat");
    test_size       = byteSize(double(fread(test_fid, probe.num_samples, 'int16')) .* 0.195);
end
...
lfp_electrical_series = types.core.ElectricalSeries( ...
    ...
    'data', lfp, ...
    'data_unit', 'uV', ...
    'filtering', '4th order Butterworth 0,1-200 Hz', ...
...
muae_electrical_series = types.core.ElectricalSeries( ...
    ...
    'data', muae, ...
    'data_unit', 'uV', ...
    'filtering', '4th order Butterworth 500-5000 Hz, full-wave rectified, then low pass 4th order Butterworth 200 Hz (DC offset high-pass 1st order Butterworth 0.1 Hz)', ...
    'timestamps', recdev.time_stamps_s_ds);
