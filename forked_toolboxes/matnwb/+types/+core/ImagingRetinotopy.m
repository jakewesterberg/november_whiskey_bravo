...
% REQUIRED PROPERTIES
properties
    axis_1_phase_map; % REQUIRED ((float32) ) Phase response to stimulus on the first measured axis.
    ...
    axis_1_phase_map_field_of_view; % REQUIRED (float32) Size of viewing area, in meters.
    axis_1_phase_map_unit; % REQUIRED (char) Unit that axis data is stored in (e.g., degrees).
    axis_1_power_map_dimension; % REQUIRED (int32) Number of rows and columns in the image. NOTE: row, column representation is equivalent to height, width.
    axis_1_power_map_field_of_view; % REQUIRED (float32) Size of viewing area, in meters.
    axis_1_power_map_unit; % REQUIRED (char) Unit that axis data is stored in (e.g., degrees).
    axis_2_phase_map; % REQUIRED ((float32) ) Phase response to stimulus on the second measured axis.
    ...
    axis_2_phase_map_field_of_view; % REQUIRED (float32) Size of viewing area, in meters.
    axis_2_phase_map_unit; % REQUIRED (char) Unit that axis data is stored in (e.g., degrees).
    axis_2_power_map_dimension; % REQUIRED (int32) Number of rows and columns in the image. NOTE: row, column representation is equivalent to height, width.
    axis_2_power_map_field_of_view; % REQUIRED (float32) Size of viewing area, in meters.
    axis_2_power_map_unit; % REQUIRED (char) Unit that axis data is stored in (e.g., degrees).
    ...
    focal_depth_image_bits_per_pixel; % REQUIRED (int32) Number of bits used to represent each value. This is necessary to determine maximum (white) pixel value.
    focal_depth_image_dimension; % REQUIRED (int32) Number of rows and columns in the image. NOTE: row, column representation is equivalent to height, width.
    focal_depth_image_field_of_view; % REQUIRED (float32) Size of viewing area, in meters.
    focal_depth_image_focal_depth; % REQUIRED (float32) Focal depth offset, in meters.
    focal_depth_image_format; % REQUIRED (char) Format of image. Right now only 'raw' is supported.
    sign_map_dimension; % REQUIRED (int32) Number of rows and columns in the image. NOTE: row, column representation is equivalent to height, width.
    sign_map_field_of_view; % REQUIRED (float32) Size of viewing area, in meters.
    ...
    vasculature_image_field_of_view; % REQUIRED (float32) Size of viewing area, in meters.
    vasculature_image_format; % REQUIRED (char) Format of image. Right now only 'raw' is supported.
end
% OPTIONAL PROPERTIES
properties
    axis_1_power_map; % OPTIONAL ((float32) ) Power response on the first measured axis. Response is scaled so 0.0 is no power in the response and 1.0 is maximum relative power.
    axis_2_power_map; % OPTIONAL ((float32) ) Power response on the second measured axis. Response is scaled so 0.0 is no power in the response and 1.0 is maximum relative power.
    focal_depth_image; % OPTIONAL ((uint16) ) Gray-scale image taken with same settings/parameters (e.g., focal depth, wavelength) as data collection. Array format: [rows][columns].
    sign_map; % OPTIONAL ((float32) ) Sine of the angle between the direction of the gradient in axis_1 and axis_2.
end
