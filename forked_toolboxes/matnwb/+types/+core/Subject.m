...
% OPTIONAL PROPERTIES
properties
    age; % OPTIONAL ((char) ) Age of subject. Can be supplied instead of 'date_of_birth'.
    date_of_birth; % OPTIONAL ((isodatetime) ) Date of birth of subject. Can be supplied instead of 'age'.
    description; % OPTIONAL ((char) ) Description of subject and where subject came from (e.g., breeder, if animal).
    genotype; % OPTIONAL ((char) ) Genetic strain. If absent, assume Wild Type (WT).
    sex; % OPTIONAL ((char) ) Gender of subject.
    species; % OPTIONAL ((char) ) Species of subject.
    strain; % OPTIONAL ((char) ) Strain of subject.
    subject_id; % OPTIONAL ((char) ) ID of animal/person used/participating in experiment (lab convention).
    weight; % OPTIONAL ((char) ) Weight at time of experiment, at time of surgery and at other important times.
end
