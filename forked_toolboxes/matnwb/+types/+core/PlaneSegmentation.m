...
% OPTIONAL PROPERTIES
properties
    image_mask; % OPTIONAL (VectorData) ROI masks for each ROI. Each image mask is the size of the original imaging plane (or volume) and members of the ROI are finite non-zero.
    pixel_mask; % OPTIONAL (VectorData) Pixel masks for each ROI: a list of indices and weights for the ROI. Pixel masks are concatenated and parsing of this dataset is maintained by the PlaneSegmentation
    pixel_mask_index; % OPTIONAL (VectorIndex) Index into pixel_mask.
    reference_images; % OPTIONAL (ImageSeries) One or more image stacks that the masks apply to (can be one-element stack).
    voxel_mask; % OPTIONAL (VectorData) Voxel masks for each ROI: a list of indices and weights for the ROI. Voxel masks are concatenated and parsing of this dataset is maintained by the PlaneSegmentation
    voxel_mask_index; % OPTIONAL (VectorIndex) Index into voxel_mask.
end
