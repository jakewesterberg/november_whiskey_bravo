...
% OPTIONAL PROPERTIES
properties
    source_channels; % OPTIONAL (DynamicTableRegion) DynamicTableRegion pointer to the channels that this decomposition series was generated from.
    source_timeseries; % OPTIONAL (TimeSeries) 
end

methods
    function obj = DecompositionSeries(varargin)
        % DECOMPOSITIONSERIES Constructor for DecompositionSeries
        varargin = [{'data_conversion' types.util.correctType(1, 'float32') 'data_resolution' types.util.correctType(-1, 'float32') 'data_unit' 'no unit'} varargin];
