...
% OPTIONAL PROPERTIES
properties
    distance; % OPTIONAL ((float32) ) Distance from camera/monitor to target/eye.
    field_of_view; % OPTIONAL ((float32) ) Width, height and depth of image, or imaged area, in meters.
    orientation; % OPTIONAL ((char) ) Description of image relative to some reference frame (e.g., which way is up). Must also specify frame of reference.
end
