...
% OPTIONAL PROPERTIES
properties
    feature_units; % OPTIONAL ((char) ) Units of each feature.
end

methods
    function obj = AbstractFeatureSeries(varargin)
        % ABSTRACTFEATURESERIES Constructor for AbstractFeatureSeries
        varargin = [{'data_conversion' types.util.correctType(1, 'float32') 'data_resolution' types.util.correctType(-1, 'float32') 'data_unit' 'see `feature_units`'} varargin];
