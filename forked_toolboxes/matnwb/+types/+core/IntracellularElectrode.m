...
% OPTIONAL PROPERTIES
properties
    filtering; % OPTIONAL ((char) ) Electrode specific filtering.
    initial_access_resistance; % OPTIONAL ((char) ) Initial access resistance.
    location; % OPTIONAL ((char) ) Location of the electrode. Specify the area, layer, comments on estimation of area/layer, stereotaxic coordinates if in vivo, etc. Use standard atlas names for anatomical regions when possible.
    resistance; % OPTIONAL ((char) ) Electrode resistance, in ohms.
    seal; % OPTIONAL ((char) ) Information about seal used for recording.
    slice; % OPTIONAL ((char) ) Information about slice used for recording.
end
