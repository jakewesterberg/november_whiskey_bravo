function [nwb, recdev] = proc_AIO(pp, nwb, recdev, rii, recording_info)

switch recording_info.Raw_Data_Format{rii}
    case 'Blackrock NSx'
        if any(recdev.local_aio)
            ...
            while ~itted_through
                jj = jj+1;
                try
                    aio_dev_list = [aio_dev_list, returnGSNum(recording_info.AIO_System, rii, jj)];
                catch
                    aio_dev_list = aio_dev_list(1:end-1);
                    itted_through = 1;
                end
            end

            local_aio = find(aio_dev_list == recdev.num);
            recdev.board_adc_data = [];
            for ii = local_aio

                temp_port_id = returnGSStr(recording_info.AIO_Port, rii, ii);

                fid = fopen([pp.CAT_DATA nwb.identifier '_dev-' num2str(recdev.num) filesep ...
                    'board-ANALOG-IN-' num2str(temp_port_id) '.dat']);
                temp_local_aio = fread(fid);
                fclose(fid);

                if size(temp_local_aio, 2) == 1
                    temp_local_aio = temp_local_aio';
                end
...
for jj = 1 : numel(adc_map)
    ...
    if strcmp(lower(adc_map(jj)), 'eye_1_x')
        ...
        eye_position = types.core.SpatialSeries( ...
            'description', 'The position of the eye. Actual sampling rate different than reported (Reported=1kHz)', ...
            'data', temp_dat, ...
        ...
        nwb.acquisition.set(['eye_1_tracking_dev_' num2str(recdev.num)], eye_tracking);
        ...
        if sum(find_p)
            ...
            nwb.acquisition.set(['pupil_1_tracking_dev_' num2str(recdev.num)], pupil_tracking);
        else
            find_p_x = find(ismember(lower(adc_map), "eye_1_pupil_x"));
            find_p_y = find(ismember(lower(adc_map), "eye_1_pupil_y"));

            temp_pdat(1,:) = downsample(recdev.board_adc_data(find_p_x, :), downsample_factor);
            temp_pdat(2,:) = downsample(recdev.board_adc_data(find_p_y, :), downsample_factor);

            pupil_position = types.core.SpatialSeries( ...
                'description', 'The dimensions of the pupil. Actual sampling rate different than reported (Reported=1kHz)', ...
                'data', temp_pdat, ...
                'starting_time_rate', 1000, ... % Hz
                'timestamps', recdev.time_stamps_s_ds, ...
                'timestamps_unit', 'seconds' ...
            ...
            pupil_tracking = types.core.EyeTracking();
            pupil_tracking.spatialseries.set('pupil_1_dimensions_data', pupil_position);
            nwb.acquisition.set(['pupil_1_tracking_dev_' num2str(recdev.num)], pupil_tracking);
    ...
    if strcmp(lower(adc_map(jj)), 'eye_2_x')
        ...
        nwb.acquisition.set(['eye_2_tracking_dev_' num2str(recdev.num)], eye_tracking);
        ...
        if sum(find_p)
            ...
            nwb.acquisition.set(['pupil_2_tracking_dev_' num2str(recdev.num)], pupil_tracking);
        else
            find_p_x = find(ismember(lower(adc_map), "eye_2_pupil_x"));
            find_p_y = find(ismember(lower(adc_map), "eye_2_pupil_y"));
            ...
            pupil_tracking = types.core.EyeTracking();
            pupil_tracking.spatialseries.set('pupil_2_dimensions_data', pupil_position);
            nwb.acquisition.set(['pupil_2_tracking_dev_' num2str(recdev.num)], pupil_tracking);
    ...
    if strcmp(lower(adc_map(jj)), 'photodiode')
        ...
        nwb.acquisition.set(['photodiode_' num2str(pd_ctr) '_tracking_dev_' num2str(recdev.num)], pd_tracking);
    ...
    if strcmp(lower(adc_map(jj)), 'reward')
        ...
        nwb.acquisition.set(['reward_' num2str(rew_ctr) '_tracking_dev_' num2str(recdev.num)], rew_tracking);
        ...
        nwb.intervals.set(['reward_' num2str(rew_ctr) '_detected_dev_' num2str(recdev.num)], trials); clear trials
    ...
    if strcmp(lower(adc_map(jj)), 'laser')
        ...
        nwb.acquisition.set(['laser_' num2str(laser_ctr) '_tracking_dev_' num2str(recdev.num)], laser_tracking);
        ...
        nwb.intervals.set(['laser_' num2str(rew_ctr) '_empirical_decode_dev_' num2str(recdev.num)], trials); clear trials
