function [nwb, recdev, probe] = proc_PRO(pp, nwb, recording_info, ii, num_recording_devices)
...
for rd = 0 : num_recording_devices-1
    ...
    switch recording_info.Raw_Data_Format{ii}
        ...
        case 'Intan'
            ...
            recdev{rd+1}.device = 'Intan';
            recdev{rd+1}.bit2volt = 0.195;
            clear in_file_path
    ...
    recdev{rd+1}.local_aio_channel = str2double(strtrim(split(recording_info.AIO_Port{ii}, ';'))) == rd;
    recdev{rd+1}.local_aio_channel = recdev{rd+1}.local_aio_channel(1:end-1);
